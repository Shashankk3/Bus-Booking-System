...
class bus_booking:
    ...
    def home_page(self):
        ...
        Label(root, text="Online Bus Booking System", font='Arial 20 bold', bg='light blue', fg='red').grid(row=2, column=2, columnspan=9, padx=w // 2.5)
        ...
        Label(root, text='\n').grid(row=5, column=8)
        Label(root, text='*For Admin Only', fg='red',font='Arial 10 bold').grid(row=6, column=8)

        root.mainloop()

    def cover_page(self):
        root = Tk()
        root.title("bus project")
        bus = PhotoImage(file='Bus_for_project.png')

        h, w = root.winfo_screenheight(), root.winfo_screenwidth()
        root.geometry('%dx%d+0+0' % (w, h))
        Label(root, text='\n\n\n\n\n\n').pack()
        Label(root, text="\n\nProject Based Learning", font='Arial 15 bold', fg='red').pack()
        Label(root, image=bus).pack()
        Label(root, text="Online Bus Booking System", font='Arial 20 bold', bg='light blue', fg='red').pack()
        Label(root, text="\n\nName: Shashank Kartikeya", font='Arial 16 bold', fg='blue').pack()
        Label(root, text="\nEr no. : 211B401", font='Arial 16 bold', fg='blue').pack()
    ...
    def journey_detail_page(self):
        ...
        def show_bus():
            ...
            if tp.isalpha() and fp.isalpha():
                if not jd=='':
                    ...
                    if len(res_route)==0:
                        showerror('no route found!','we are not running on this route')
                    else:
                        for i in res_route:
                            for j in i:
                                val_route = str(j)
                        ...
                        if len(res_bid)==0:
                            showerror('no bus available!','we have not any bus on this route!')
                        else:
                            val_bid = []
                            for i in res_bid:
                                for j in i:
                            ...
                            if len(b)==0:
                                showerror('no running bus',"no running bus")
                            else:
                                ...
                                def proceed():
                                    ...
                                    Label(root,text='Fill passenger details to book the bus',font='Arial 18 bold').grid(row=11,columnspan=12)
                                    ...
                                    Label(root, text='Number of seats', font='Arial 10 bold').grid(row=13, column=7)
                                            ...
                                            if len(name)>0 and len(name)<20:
                                                if age>0 and age<150:
                                                    if seats>0 and seats<101:
                                                        ...
                                                        con.commit()
                                                        showinfo("!yeah","booking successfull")

                                                    else:
                                                        showerror("! booking limit exceed","please you can only book upto 100  seats")
                                                else:
                                                    showerror("! incorrect age","please enter valid age")
                                            else:
                                                showerror("! incorrect name","please enter valid name")
                                        else:
                                            showerror("! invalid mobile no","please enter valid mobile no")
                    ...
                    showerror('Error','please Enter correctly')
        ...
        Label(root,text="enter your date with YYYY-MM-DD").grid(row=5,column=8)

        Button(root, text='Show Bus', bg='green', fg='black', font='Arial 12 bold',command=show_bus).grid(row=4, column=9, padx=30)

        home = PhotoImage(file='home.png')

        Button(root, image=home,command=journey_to_home).grid(row=3, column=9)
    ...
    def check_booking_page(self):
        ...
        def check_tk():
            ...
            if len(mobile)==10 and mobile.isdigit():
                ...
                if(res_tkt==[]):
                    ...
                    res_ref=cur.fetchall()
                    b_ref=res_ref[0][0]
                    Label(root,text="TICKET", font='Arial 14 bold').grid(row=6,columnspan=12 )
                    ...
                    Label(frame,text="booked date= " + book_date, font='Arial 12 bold', fg='black').grid(row=10, column=5)
        ...
        Label(root, text="Check Your Booking", font='Arial 22 bold', bg='green3', fg='dark green').grid(row=3, column=0, pady=20, columnspan=12, padx=600)
        ...
        Button(root, text='Check Booking', font='Arial 12',command=check_tk).grid(row=4, column=7)
    ...
    def db_add_page(self):
        ...
        Label(root, text='\n').grid(row=3, column=2)
        Label(root, text="Add New Details To Database", font='Arial 16 bold', bg='white', fg='green3').grid(row=4,column=2,columnspan=9,padx=w // 2.5)
        Label(root, text='\n').grid(row=5, column=2)

        Button(root, text='New Operator', fg='black', bg='light green', font='Arial 12',command=db_add_to_add_op).grid(row=6, column=1,columnspan=7)
                        ...
                        if iphone.isnumeric() and len(iphone) == 10:
                            if len(iemail) > 0 and len(iemail) < 30:
                                if (iid,) in res:
                                    ...
                                    data="op_id="+iid+"     op name="+iname+"     op add="+iaddress+"     op phone="+iphone+"     op email="+iemail
                                ...
                                showerror("invalid input?", "please enter email correctly")
                        else:
                            showerror("invalid input?", "please enter phone correctly")
                    else:
                        showerror("invalid input?", "please enter address correctly")
                else:
                    showerror("invalid input?", "please enter name correctly")
            else:
                showerror("invalid input?", "please enter id correctly")
    ...
    def add_bus_page(self):
        ...
        Button(root, text="Add Bus", font='Arial 12 bold', bg='green2', fg='black',command=bus_add).grid(row=5, column=4, pady=20,columnspan=4)
    ...
    def add_route_page(self):
        ...
        def add_route():
            ...
            if (route_id,) in res:
                ...
                data="route_id="+route_id+"     station_start="+start_station+"      startstation_id="+start_id+"     endstation="+end_station+"     endstationid="+ end_id
        ...
        Button(root, text="Add Route", font='Arial 12 bold', bg='green2', fg='black',command=add_route).grid(row=3, column=7, pady=2,padx=10)
        Button(root, text="Delete Route", font='Arial 12 bold', bg='green2', fg='black',command=delete_route).grid(row=3, column=9, pady=20,padx=10)
    ...
    def add_bus_running_page(self):
        ...
        def add_run():
            ...
            data="bus_id="+bid+"     rundate="+run_date+"      seat available="+s_avail
        ...
        def del_run():
            ...
            showinfo('sucess','run deleted successfully!!')
            con.commit()
            

        Label(root, image=bus).grid(row=0, column=0, columnspan=12)
        Label(root, text="Online Bus Booking System", font='Arial 28 bold', bg='sky blue', fg='red').grid(row=1,column=0,columnspan=12)
        Label(root, text="Add Bus Running Details", font='Arial 20 bold', bg='gainsboro', fg='green4').grid(row=2,column=0,pady=20,columnspan=12)
        ...
        Label(root,text="*date format YYYY-MM-DD",font='Arial 12 bold').grid(row=4,column=3)
        ...
        Button(root, text="Add Run", font='Arial 12 bold', bg='green2', fg='black',command=add_run).grid(row=3, column=6, pady=20,padx=10)
        Button(root, text="Delete Run", font='Arial 12 bold', bg='green2', fg='black',command=del_run).grid(row=3, column=7, pady=20,padx=10)

        home = PhotoImage(file='home.png')

        Button(root, image=home,command=add_bus_running_to_home,bo=3).grid(row=4, column=7)
